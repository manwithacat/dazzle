...
export class RealtimeClient {
  constructor(url, options = {}) {
    ...
    if (typeof window !== 'undefined' && window.DNR && window.DNR.createSignal) {
      const [connSignal, setConnSignal] = window.DNR.createSignal(false);
...
export class PresenceManager {
  ...
  async join(resource, metadata = {}) {
    ...
    if (!this.resources.has(resource)) {
      ...
      if (typeof window !== 'undefined' && window.DNR && window.DNR.createSignal) {
        const [getter, setter] = window.DNR.createSignal([]);
...
export function createRealtimeClient(url, options = {}) {
  ...
  // Get API client from DNR if available
  const apiClient = (typeof window !== 'undefined' && window.DNR?.api) ||
    { create: () => {}, update: () => {}, remove: () => {} };

  return {
    client,
    optimistic: new OptimisticManager(apiClient, client),
