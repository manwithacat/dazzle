...
class FileMetadataStore:
    ...
    def _init_db(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                CREATE TABLE IF NOT EXISTS dnr_files (
                    id TEXT PRIMARY KEY,
            ...
            conn.execute("""
                ...
                ON dnr_files(entity_name, entity_id)
            """)
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_files_field
                ON dnr_files(entity_name, field_name)
            """)
            conn.commit()
    ...
    def save(self, metadata: FileMetadata) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                """
                INSERT OR REPLACE INTO dnr_files
    ...
    def get(self, file_id: UUID | str) -> FileMetadata | None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            row = conn.execute(
                "SELECT * FROM dnr_files WHERE id = ?",
                (str(file_id),),
            ).fetchone()
    ...
    def delete(self, file_id: UUID | str) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "DELETE FROM dnr_files WHERE id = ?",
                (str(file_id),),
            )
            conn.commit()
