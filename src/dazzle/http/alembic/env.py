...
def _process_revision_directives(context: Any, revision: Any, directives: list[Any]) -> None:
    ...
    # #1427: scope autogenerate to additive ops so a routine `db revision` can never
    # emit a destructive whole-schema rewrite (text→uuid PK churn / live-column drops)
    # from metadata-vs-DB diff noise. Opt out with DAZZLE_ALEMBIC_ALLOW_DESTRUCTIVE=1
    # for a deliberately destructive migration (then USING-clause injection below still
    # applies to the retained AlterColumns).
