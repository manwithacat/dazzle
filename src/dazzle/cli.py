...
def dnr_serve(
    ...
    detach: bool = typer.Option(
        ...
        "--detach",
        "-d",
        help="Run Docker container in background (detached mode)",
    ),
) -> None:
    """
    Serve DNR app (backend API + UI with live data).

    By default, runs in Docker for consistent development environment.
    Use --local to run directly with Python (requires all dependencies).
    ...
    if not local and not ui_only:
        try:
            from dazzle_dnr_ui.runtime import is_docker_available, run_in_docker

            if is_docker_available():
                typer.echo("Running in Docker mode (use --local to run without Docker)")
