...
{self.spec.title or "Generated from DAZZLE specification."}
...
class EjectionResult:
    ...
    def merge_generator_result(
        ...
        Note: GeneratorResult writes files directly to disk via add_file().
        We just track the paths here (content is already written).
        """
        for path in result.files_created:
            # Mark file as handled - content already written by generator
            if path not in self.files:
                self.files[path] = ""  # Placeholder to track that file exists
    ...
    def write_files(self) -> None:
        ...
        Skips files with empty content (already written by generators).
        """
        for path, content in self.files.items():
            if content:  # Skip generator-written files (empty placeholder)
