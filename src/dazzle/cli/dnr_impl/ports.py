"""
Port allocation for DNR serve.

Provides deterministic port assignment based on project name to prevent
collisions when running multiple DNR instances in parallel.
...
class PortAllocation(NamedTuple):
    """Allocated ports for a DNR instance."""
...
def allocate_ports(
    ...
    """
    Allocate ports for a DNR instance.
...
def write_runtime_file(project_root: Path, allocation: PortAllocation) -> Path:
    ...
    the ports for a running DNR instance.
