"""
DAZZLE Semantic Layout Engine.

Deterministic, compiler-based UI layout planning from workspace specifications.

Key components:
- Stage selection (select_archetype.py)
- Surface allocation (allocate.py)
- Persona adjustments (adjust.py)
- Layout plan assembly (plan.py)
"""

from dazzle.layout.archetypes import (
    ARCHETYPE_DEFINITIONS,
    ArchetypeDefinition,
)
from dazzle.layout.cache import LayoutPlanCache, get_layout_cache
from dazzle.layout.converter import (
    convert_workspace_to_layout,
    convert_workspaces_to_layouts,
    enrich_app_spec_with_layouts,
)
from dazzle.layout.plan import build_layout_plan
from dazzle.layout.select_archetype import (
    SelectionExplanation,
    StageScore,
    explain_stage_selection,
    select_stage,
)
from dazzle.layout.types import (
    AttentionSignalKind,
    LayoutArchetype,
    LayoutPlan,
    LayoutSignal,
    LayoutSurface,
    PersonaLayout,
    WorkspaceLayout,
)
from dazzle.layout.variants import (
    VARIANT_CONFIGS,
    EngineVariant,
    VariantConfig,
    get_grid_columns,
    get_variant_config,
    get_variant_for_persona,
)

__all__ = [
    # Core functions
    "build_layout_plan",
    "select_stage",
    "explain_stage_selection",
    "SelectionExplanation",
    "StageScore",
    # DSL conversion
    "convert_workspace_to_layout",
    "convert_workspaces_to_layouts",
    "enrich_app_spec_with_layouts",
    # Caching
    "LayoutPlanCache",
    "get_layout_cache",
    # Archetype definitions
    "ARCHETYPE_DEFINITIONS",
    "ArchetypeDefinition",
    # Engine variants
    "EngineVariant",
    "VariantConfig",
    "VARIANT_CONFIGS",
    "get_variant_config",
    "get_variant_for_persona",
    "get_grid_columns",
    # IR types (re-exported for convenience)
    "LayoutSignal",
    "AttentionSignalKind",
    "LayoutArchetype",
    "LayoutPlan",
    "LayoutSurface",
    "PersonaLayout",
    "WorkspaceLayout",
]
