...
- a **supporting widget** (a chart/metric/summary, or any region with declared
  `aggregates`) self-**suppresses** when empty — an empty chart is pure noise;
...
def resolve_when_empty(region: Any) -> WhenEmpty:
    ...
    - Unset → the default-flip: `message` when the author declared an
      `empty_message` or the region is primary content; `suppress` for an empty
      supporting widget (chart/metric/aggregate).
    """
