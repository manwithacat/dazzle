...
section rather than scanning the 3,200+ lines):

  L 127  Contracts          HandlerConfig, RouteSpec (shared CRUD bundle)
  L 238  Request utils      _is_htmx_request, _wants_html, _forbidden_detail,
                             _htmx_current_url, _htmx_parent_url,
                             _parse_request_body
  L 307  Audit / access     _normalize_role, _build_access_context,
                             _compute_field_changes, _log_audit_decision
  L 446  Auth wrapper       _wrap_with_auth → _build_cedar_handler /
                             _build_auth_handler / _build_noauth_handler
  L 801  List handler       create_list_handler, _list_handler_body,
                             _is_field_condition
  L1405  Read handler       create_read_handler
  L1577  Ref injection      resolve_backed_entity_refs, inject_current_user_refs
  L1712  Write handlers     create_create_handler, create_update_handler,
                             create_delete_handler, create_custom_handler
  L2146  Graph helpers      _materialize_graph, _neighborhood_handler_body,
                             create_neighborhood_handler  (#619 phases 3–4)
  L2389  Graph algorithms   create_shortest_path_handler,
                             create_components_handler  (#619 phase 4)
  L2522  RouteGenerator     Primary class — wires all factories into APIRouter
  L3085  Convenience        generate_crud_routes (thin wrapper)
