...
if TYPE_CHECKING:
    ...
    from dazzle.back.runtime.sa_schema import build_metadata as build_metadata
...
def _get_create_bus() -> object:
    from dazzle.back.events.tier import create_bus

    return create_bus


def _get_MigrationAction() -> object:  # noqa: N802
    from dazzle.back.runtime.migrations import MigrationAction
...
def __getattr__(name: str) -> object:
    ...
    if loader is not None:
        ...
        return val
    raise AttributeError(f"module 'dazzle.back' has no attribute {name!r}")
