...
- `render_agents_md(commands, ctx)` — canonical AGENTS.md project root file
- `render_claude_md_section(commands, ctx)` — thin section for the CLAUDE.md adapter
- `render_command_shim(cmd)` — one-line discovery stub for `.claude/commands/`
"""
...
a session-tier subagent (subagent-dispatch) — the cognitive work is billed
to the outer assistant's host subscription, not the metered SDK path.
...
One cycle per invocation (~15 minutes). Re-run on a scheduled cadence
(scheduled-loop) for continuous improvement.
...
# ---------------------------------------------------------------------------
# Aggregate templates — AGENTS.md (canonical) and CLAUDE.md adapter section
# ---------------------------------------------------------------------------


def render_command_shim(cmd: Any) -> str:
    """One-line discovery stub for `.claude/commands/<name>.md`.

    Full workflow bodies live under `.agents/skills/<name>/SKILL.md` (open-standard
    format). Shims keep slash-command discovery working without duplicating policy.
    """
    return (
        f"<!-- dazzle-agent-command:{cmd.name}:v{cmd.version} -->\n"
        f"Read and follow `.agents/skills/{cmd.name}/SKILL.md` "
        f"(portable home for this workflow).\n"
    )


def render_skill_document(cmd: Any, ctx: dict[str, Any]) -> str:
    """Full portable skill body with open-standard frontmatter for `.agents/skills/`."""
    body = render_skill(cmd, ctx)
    header = (
        f"<!-- dazzle-agent-command:{cmd.name}:v{cmd.version} -->\n"
        f"---\n"
        f"name: {cmd.name}\n"
...
AGENTS_SYNC_BEGIN = "<!-- dazzle-agent-sync:begin -->"
AGENTS_SYNC_END = "<!-- dazzle-agent-sync:end -->"


def render_agents_sync_block(
    commands: list[tuple[Any, bool, str | None]],
    ctx: dict[str, Any],
) -> str:
    """Render the managed AGENTS.md block (workflows + command catalogue).

    Bounded by ``AGENTS_SYNC_BEGIN`` / ``AGENTS_SYNC_END`` so re-sync can replace
    only this region and preserve blank-template / user project policy above it.
    """
    ...
    for cmd, available, _reason in commands:
        ...
        workflow_lines.append(
            f"- **{cmd.name}** — {cmd.description} (`.agents/skills/{cmd.name}/SKILL.md`)"
        )
        loop_block = ""
        if cmd.loop:
            loop_block = (
        ...
        available_blocks.append(
            ...
            f"- **Skill body**: `.agents/skills/{cmd.name}/SKILL.md`\n"
            f"{loop_block}"
    ...
    workflows = (
        "\n".join(workflow_lines) if workflow_lines else "(none yet — add entities/surfaces)"
    )
    available_section = "\n".join(available_blocks) if available_blocks else "(none)"

    return f"""{AGENTS_SYNC_BEGIN}
## Workflows

Reusable workflows live in `.agents/skills/<name>/SKILL.md` (open-standard format).
Discovery shims for slash-command harnesses live under `.claude/commands/`.
Regenerated by `dazzle agent sync` — do not hand-edit this block.
...
def render_agents_md(
    ...
    """Render a full AGENTS.md when the project has none yet."""
    project_name = ctx.get("project_name", "project")
    block = render_agents_sync_block(commands, ctx)
    return f"""# AGENTS.md

Canonical project instructions for **all** coding agents (any harness).
Workflows are maintained by `dazzle agent sync` inside the marked block below.
Add project-specific notes outside that block (they are preserved on re-sync).

## Project Overview

**{project_name}** — a Dazzle app (DSL-first). Domain logic lives in `dsl/*.dsl`;
the runtime executes the linked AppSpec directly.
...
def render_claude_md_section(
    ...
    """Render the managed section of the thin `.claude/CLAUDE.md` adapter."""
    available_lines = [
        f"- `/{cmd.name}` — {cmd.description} (body: `.agents/skills/{cmd.name}/SKILL.md`)"
        for cmd, available, _reason in commands
        if available
    ]
    loop_lines = [
...
Synced by `dazzle agent sync`. Portable bodies live in `.agents/skills/`;
this directory only holds discovery shims under `commands/`.
