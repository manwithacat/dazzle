...
def clerk_fts_snippet_field_label(field_key: Any) -> str:
    """Schema key → clerk FTS snippet field (oral #180).

    ``first_name`` dumped as ``first_name:`` on search_box hits while the
    directory already says ``First Name``. Leftover junk stays put.
...
def _ref_display_name(value: Any, fallback: str = "") -> str:
    ...
    result = (
        ...
        or value.get("email")
        or value.get("code")
        # Document identifiers (Goal B document depth — invoices, POs, tickets)
        or value.get("invoice_number")
        or value.get("document_number")
        or value.get("number")
        or value.get("po_number")
