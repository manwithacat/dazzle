...
def extract_pitch_context(project_root: Path, spec: PitchSpec) -> PitchContext:
    ...
    try:
        ...
        try:
            ...
            logger.warning("Failed to load stories for pitch context", exc_info=True)

        # Extract integrations
        try:
            ctx.integrations = [i.name for i in appspec.integrations]
        except Exception:
            logger.warning("Failed to extract integrations", exc_info=True)

        # Extract services
        try:
            ctx.services = [s.name for s in appspec.domain_services]
        except Exception:
            logger.warning("Failed to extract services", exc_info=True)

        # Count ledgers (TigerBeetle)
        try:
            ctx.ledger_count = len(appspec.ledgers)
        except Exception:
            logger.warning("Failed to count ledgers", exc_info=True)

        # Count processes
        try:
            ctx.process_count = len(appspec.processes)
        except Exception:
            logger.warning("Failed to count processes", exc_info=True)
        ...
        try:
            ...
            logger.warning("Failed to load E2E flows", exc_info=True)
        ...
        try:
            ...
            logger.warning("Failed to analyze infra requirements", exc_info=True)
