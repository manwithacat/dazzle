...
"""
    ...
    def _generate_list_page(self, surface: ir.SurfaceSpec, result: GeneratorResult) -> None:
        ...
        ux_block = surface.ux
        empty_message = (
            ux_block.empty_message
            if ux_block and ux_block.empty_message
            else f"No {entity_name}s found"
        )
...
"""
    ...
    def _generate_delete_button(
        self, entity_name: str, snake_name: str, result: GeneratorResult
    ) -> None:
        """Generate delete button component."""
        pascal_entity = self._pascal_case(entity_name)
...
"""
    ...
    def _build_form_fields(
        self, entity: ir.EntitySpec, surface: ir.SurfaceSpec, is_create: bool
    ) -> str:
        ...
        for field in entity.fields:
            ...
            if field.type.kind == ir.FieldTypeKind.TEXT:
                ...
                options = "\n                    ".join(
                    f'<option value="{v}">{v}</option>' for v in field.type.enum_values
                )
                input_elem = f'''<Select
                  id="{camel_name}"
                  name="{camel_name}"
