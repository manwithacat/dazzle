...
class ActionsGenerator(Generator):
    ...
    def _generate_entity_actions(
        self, entity: ir.EntitySpec, result: GeneratorResult
    ) -> None:
        ...
        # Fields to include in create (exclude auto-generated)
        create_fields = [
            f for f in entity.fields
            if not f.is_primary_key or f.type.kind != ir.FieldTypeKind.UUID
            if ir.FieldModifier.AUTO_ADD not in f.modifiers and ir.FieldModifier.AUTO_UPDATE not in f.modifiers
        ]

        # Fields to include in update (same as create)
...
'''
    ...
    def _generate_actions_index(self, result: GeneratorResult) -> None:
        ...
        for entity in self.spec.domain.entities:
            ...
            exports.append(f'export {{ get{pascal_name}s, get{pascal_name}, create{pascal_name}, update{pascal_name}, delete{pascal_name}, search{pascal_name}s }} from "./{snake_name}";')
