...
class FormsGenerator(Generator):
    ...
    def _build_forms_code(self) -> str:
        ...
        for entity in self.spec.domain.entities:
            ...
            if create_surface:
                lines.append(
                    self._generate_surface_form(
                        entity, create_surface, f"{entity.name}CreateForm", "creation"
                    )
                )
                lines.append("")
                lines.append("")
            ...
            if edit_surface:
                lines.append(
                    self._generate_surface_form(
                        entity, edit_surface, f"{entity.name}Form", "editing"
                    )
                )
            else:
    ...
    def _generate_surface_form(
        self, entity: ir.EntitySpec, surface: ir.SurfaceSpec, form_class_name: str, purpose: str
    ) -> str:
                ...
                for element in section.elements:
                    if element.field_name:
                        ...
                        if entity_field:
                            ...
                            if not (
                                ir.FieldModifier.AUTO_ADD in entity_field.modifiers
                                or ir.FieldModifier.AUTO_UPDATE in entity_field.modifiers
                            ):
