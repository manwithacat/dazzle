...
class TestGenerator(Generator):
    ...
    def _generate_model_test_class(self, entity: ir.EntitySpec) -> str:
        ...
        for field in entity.fields:
            if field.is_required and not field.is_primary_key and field.type.kind != ir.FieldTypeKind.REF:
    ...
    def _generate_setUp_for_entity(self, entity: ir.EntitySpec, fk_fields: List[ir.FieldSpec]) -> List[str]:
        ...
        for fk_field in fk_fields:
            ...
            if ref_entity_name not in referenced_entities:
                ...
                if ref_entity:
                    ...
                    lines.append(f'        self.{ref_entity_name.lower()} = {ref_entity_name}.objects.create(')
    ...
    def _generate_test_create(self, entity: ir.EntitySpec) -> List[str]:
        ...
        for field_name, value in list(create_params.items())[:3]:  # Check first 3 fields
            if 'self.' not in value:
                lines.append(f'        self.assertEqual({entity.name.lower()}.{field_name}, {value})')

        return lines

    def _generate_test_required_field(self, entity: ir.EntitySpec, field: ir.FieldSpec) -> List[str]:
    ...
    def _generate_test_foreign_key(self, entity: ir.EntitySpec, field: ir.FieldSpec) -> List[str]:
        ...
        lines.append(f'            {param_str}')
        lines.append('        )')
        lines.append(f'        self.assertEqual({entity.name.lower()}.{field.name}, self.{ref_entity.lower()})')
    ...
    def _generate_test_str_method(self, entity: ir.EntitySpec) -> List[str]:
        ...
        lines.append(f'            {param_str}')
        lines.append('        )')
        lines.append(f'        # __str__ should return a non-empty string')
    ...
    def _get_field_by_name(self, entity: ir.EntitySpec, field_name: str) -> Optional[ir.FieldSpec]:
    ...
    def _get_minimal_create_params(self, entity: ir.EntitySpec, use_self: bool = False, exclude: List[str] = None) -> Dict[str, str]:
        ...
        for assertion in assertions:
            ...
            if kind == ir.TestAssertionKind.STATUS:
                ...
                if assertion.expected_value == "success":
                    lines.append(f'        # Expecting success (object should exist)')
                ...
                if operator == ir.TestComparisonOperator.EQUALS:
                    lines.append(f'        self.assertEqual({result_var}.count(), {expected_value})')
                elif operator == ir.TestComparisonOperator.GREATER_THAN:
                    lines.append(f'        self.assertGreater({result_var}.count(), {expected_value})')
                elif operator == ir.TestComparisonOperator.LESS_THAN:
