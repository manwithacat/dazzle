"""
Unified field value generator for fixtures, mock data, and tests.
...
This module lives in ``dazzle.core`` because it depends only on the IR
(``FieldSpec``, ``FieldTypeKind``) and is consumed by both the UI runtime
(mock data previews) and the testing layer.
