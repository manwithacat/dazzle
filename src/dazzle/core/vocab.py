...
class VocabParameter(BaseModel):
    ...
    def validate_type(cls, v: str) -> str:
        ...
        if v not in valid_types:
            raise ValueError(f"Invalid parameter type '{v}'. Must be one of: {', '.join(valid_types)}")
        return v
...
class VocabEntry(BaseModel):
    ...
    parameters: List[VocabParameter] = Field(default_factory=list, description="Parameter definitions")
    ...
    def get_param(self, name: str) -> Optional[VocabParameter]:
...
def create_empty_manifest(app_id: str, dsl_core_version: str = "1.0.0") -> VocabManifest:
    ...
    return VocabManifest(
        version="1.0.0",
        app_id=app_id,
        dsl_core_version=dsl_core_version,
        entries=[]
    )
