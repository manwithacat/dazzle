...
Stories are NOT part of the DSL syntax - they are runtime artifacts
stored as JSON in .dazzle/stories/.
"""
...
class StorySpec(BaseModel):
    ...
    - Proposed by an LLM agent based on DSL analysis
    - Reviewed and edited by humans
    - Used to generate implementation stubs
    ...
    Attributes:
        ...
        preconditions: Conditions that must be true before behavior runs
        happy_path_outcome: Statements that must be true after success
    ...
    Example:
        StorySpec(
            ...
            preconditions=["Invoice.status changes from 'Draft' to 'Sent'"],
            happy_path_outcome=["Invoice is recorded as 'Sent' with timestamp"],
            side_effects=["send_invoice_email_to_client"],
            constraints=["Do not send if already 'Sent'"],
            variants=["If client email missing, flag for manual follow-up"],
        )
    """
    ...
    preconditions: list[str] = Field(
        default_factory=list, description="Conditions that must be true before"
    )
    happy_path_outcome: list[str] = Field(
        default_factory=list, description="Statements true after success"
    )
