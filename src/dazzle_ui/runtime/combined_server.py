...
if TYPE_CHECKING:
    ...
    from dazzle_ui.runtime.hot_reload import HotReloadManager
...
class DNRCombinedServer:
    ...
    def _start_hot_reload(self) -> None:
        ...
        if self.watch_source:
            msg += " + source files"
        print(f"[Dazzle] Hot reload: ENABLED (watching {msg})")
    ...
    def _start_backend(self) -> None:
        ...
        try:
            ...
            print("[Dazzle] Warning: dazzle_back not available, skipping backend")
            return
        ...
        def run_backend() -> None:
            try:
                ...
                print("[Dazzle] Warning: uvicorn not available, backend disabled")
            except OSError as e:
                if e.errno == 48 or "address already in use" in str(e).lower():
                    print(f"\n[Dazzle] ERROR: Backend port {self.backend_port} is already in use.")
                    print(
                        "[Dazzle] Stop the other process or use --api-port to specify a different port."
                    )
                    print(f"[Dazzle] Hint: lsof -i :{self.backend_port} | grep LISTEN")
                else:
                    print(f"[Dazzle] Backend error: {e}")
            except Exception as e:
                print(f"[Dazzle] Backend error: {e}")
        ...
        print(f"[Dazzle] Database: {self.db_path}")
    ...
    def _start_frontend(self) -> None:
        ...
        if self.appspec:
            try:
                ...
                print(f"[Dazzle] Template mode: ENABLED ({len(page_contexts)} page routes)")
            except Exception as e:
                print(f"[Dazzle] Template compilation failed, using SPA: {e}")
        ...
        if self.sitespec_data:
            ...
            if site_routes:
                print(f"[Dazzle] Site pages: {', '.join(sorted(site_routes))}")
        else:
        ...
        try:
            ...
            if e.errno == 48 or "address already in use" in str(e).lower():
                print(f"\n[Dazzle] ERROR: Port {self.frontend_port} is already in use.")
                print("[Dazzle] Stop the other process or use --port to specify a different port.")
                print(f"[Dazzle] Hint: lsof -i :{self.frontend_port} | grep LISTEN")
                raise SystemExit(1)
            raise

        frontend_url = f"http://{self.frontend_host}:{self.frontend_port}"
        print(f"[Dazzle] Frontend: {_clickable_url(frontend_url)}")
        print()
