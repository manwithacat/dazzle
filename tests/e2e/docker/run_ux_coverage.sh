...
services:
  dazzle-app:
    ...
    environment:
      - DAZZLE_ENV=test
      - DAZZLE_HOST=0.0.0.0
      - DAZZLE_API_PORT=8000
      - DAZZLE_FRONTEND_PORT=3000
    healthcheck:
  ...
  playwright:
    ...
    environment:
      - DAZZLE_BASE_URL=http://dazzle-app:8000
      - DAZZLE_UI_URL=http://dazzle-app:3000
...
echo "Waiting for Dazzle app to be healthy..."
for i in {1..60}; do
    if docker compose -f "$COMPOSE_FILE" ps | grep -q "healthy"; then
        echo "Dazzle app is healthy!"
        break
    fi
    if [[ $i -eq 60 ]]; then
        echo "Timeout waiting for Dazzle app"
        docker compose -f "$COMPOSE_FILE" logs dazzle-app
        exit 2
    fi
    sleep 1
done
