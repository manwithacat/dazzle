...
def load_coverage_report(coverage_path: Path) -> Optional[dict]:
...
def get_dsl_snippet(example_name: str, project_root: Path) -> Optional[str]:
    ...
    if coverage:
        ...
        if "component_coverage" in coverage:
            ...
            if isinstance(comp_cov, dict):
                tested = sum(1 for c in comp_cov.values() if isinstance(c, dict) and c.get("tested", False))
                total = len(comp_cov)
...
def main():
    ...
    parser.add_argument(
        "--example",
        required=True,
        help="Name of the example project"
    )
    parser.add_argument(
        "--screenshots",
        required=True,
    ...
    parser.add_argument(
        "--coverage",
        required=True,
        help="Path to UX coverage JSON report"
    )
    parser.add_argument(
        "--output",
        required=True,
