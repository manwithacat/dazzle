...
def _mock_appspec_with_stories(stories: list[Any]) -> MagicMock:
    """Create a mock appspec with the given stories."""
    app_spec = MagicMock()
    app_spec.stories = stories
    return app_spec
...
class TestWallStoriesHandler:
    ...
    @patch("dazzle.mcp.server.handlers.stories.load_project_appspec")
    def test_groups_by_coverage_status(
        ...
        mock_load.return_value = _mock_appspec_with_stories(MOCK_STORIES)
    ...
    def test_no_stories(
        ...
        mock_load.return_value = _mock_appspec_with_stories([])
