...
def test_live_support_tickets_empty_region_not_recommended() -> None:
    ...
    snap = snapshot()
    assert ["support_tickets", "empty_region_honesty"] in snap.saturated_cells
    rec = snap.recommend
    if rec is not None:
...
def test_live_hr_records_org_not_recommended() -> None:
    """in_tree + apex_people + reporting_seat saturates hr org (oral #13/#23)."""
    from scripts.interesting_product_portfolio import snapshot

    snap = snapshot()
    assert ["hr_records", "org_structure"] in snap.saturated_cells
    rec = snap.recommend
    if rec is not None:
        assert not (rec["app"] == "hr_records" and rec["depth_id"] == "org_structure")
...
def test_live_invoice_ops_document_not_recommended() -> None:
    ...
    snap = snapshot()
    assert ["invoice_ops", "document"] in snap.saturated_cells
    rec = snap.recommend
    if rec is not None:
        assert not (rec["app"] == "invoice_ops" and rec["depth_id"] == "document")
