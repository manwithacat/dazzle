...
def test_ticket_queue_critical_needs_reply_trail() -> None:
    ...
    region = block.split("\n  critical_needs_reply:\n", 1)[1].split(
        "\n  # Peer-pack conversation upgrade (cycle 2013)", 1
    )[0]
    assert "source: Comment" in region
    assert (
    ...
    assert "channel =" not in region
    assert "customer_tone" not in region
    # Focus later prefers frustrated_awaiting_customer (cycle 2007); critical region + metric remain.
    manager = text.split("workspace manager_ops", 1)[1].split("workspace agent_dashboard", 1)[0]
    assert "\n  critical_needs_reply:\n" in manager
...
def test_ticket_queue_urgent_awaiting_customer_trail() -> None:
    ...
    manager = text.split("workspace manager_ops", 1)[1].split("workspace agent_dashboard", 1)[0]
    assert "\n  urgent_awaiting_customer:\n" in manager
    # Focus later prefers email_awaiting_customer (cycle 2015); region + metric remain.
    assert (
        "urgent_awaiting_customer: count(Comment where customer_tone = urgent and ball_in_court = customer"
        in manager
    )
...
def test_ticket_queue_frustrated_awaiting_customer_trail() -> None:
    """Cycle 2007: Intercom/Zendesk frustrated still waiting on customer (tone×customer-ball).

    customer_tone=frustrated AND ball_in_court=customer — not full awaiting_customer,
    not agent frustrated_needs_reply, not urgent_awaiting_customer re-stack.
    """
    text = APP.read_text()
    block = text.split("workspace ticket_queue", 1)[1].split("workspace manager_ops", 1)[0]
    assert (
        "frustrated_awaiting_customer: count(Comment where customer_tone = frustrated and ball_in_court = customer"
        in block
    )
    ...
    manager = text.split("workspace manager_ops", 1)[1].split("workspace agent_dashboard", 1)[0]
    assert "\n  frustrated_awaiting_customer:\n" in manager
    assert "frustrated_awaiting_customer" in manager.split("focus:", 1)[1].split("\n", 1)[0]
    assert (
        "frustrated_awaiting_customer: count(Comment where customer_tone = frustrated and ball_in_court = customer"
        in manager
    )
...
def test_ticket_queue_critical_awaiting_customer_trail() -> None:
    """Cycle 2013: Zendesk/Service Cloud P1 still waiting on customer (critical×customer-ball).

    escalation=critical AND ball_in_court=customer — not full awaiting_customer,
    not agent critical_needs_reply, not raised×customer re-stack.
    """
    text = APP.read_text()
    block = text.split("workspace ticket_queue", 1)[1].split("workspace manager_ops", 1)[0]
    assert (
        "critical_awaiting_customer: count(Comment where escalation = critical and ball_in_court = customer"
        in block
    )
    ...
    manager = text.split("workspace manager_ops", 1)[1].split("workspace agent_dashboard", 1)[0]
    assert "\n  critical_awaiting_customer:\n" in manager
    assert "critical_awaiting_customer" in manager.split("focus:", 1)[1].split("\n", 1)[0]
    assert (
        "critical_awaiting_customer: count(Comment where escalation = critical and ball_in_court = customer"
        in manager
    )
...
def test_ticket_queue_email_awaiting_customer_trail() -> None:
    """Cycle 2015: Zendesk/Front email still waiting on customer (channel×customer-ball).

    channel=email AND ball_in_court=customer — not full email_live, not agent
    email_needs_reply, not tone/escalation×customer re-stack.
    """
    text = APP.read_text()
    block = text.split("workspace ticket_queue", 1)[1].split("workspace manager_ops", 1)[0]
    assert (
        "email_awaiting_customer: count(Comment where channel = email and ball_in_court = customer"
        in block
    )
    region = block.split("\n  email_awaiting_customer:\n", 1)[1].split(
        "\n  # Peer-pack conversation upgrade (cycle 2020)", 1
    )[0]
    assert "source: Comment" in region
    assert "filter: channel = email and ball_in_court = customer and is_internal = false" in region
    assert "display: conversation" in region
    # Channel×customer-ball compound — not agent email_needs_reply, not tone/escalation.
    assert "ball_in_court = agent" not in region
    assert "customer_tone" not in region
    assert "escalation" not in region
    assert "channel = portal" not in region
    manager = text.split("workspace manager_ops", 1)[1].split("workspace agent_dashboard", 1)[0]
    assert "\n  email_awaiting_customer:\n" in manager
    assert "email_awaiting_customer" in manager.split("focus:", 1)[1].split("\n", 1)[0]
    assert (
        "email_awaiting_customer: count(Comment where channel = email and ball_in_court = customer"
        in manager
    )
...
def test_ticket_queue_chat_awaiting_customer_trail() -> None:
    """Cycle 2020: Intercom/Front chat still waiting on customer (channel×customer-ball).

    channel=chat AND ball_in_court=customer — not full chat_live, not agent
    chat_needs_reply, not email_awaiting_customer re-stack.
    """
    text = APP.read_text()
    block = text.split("workspace ticket_queue", 1)[1].split("workspace manager_ops", 1)[0]
    assert (
        "chat_awaiting_customer: count(Comment where channel = chat and ball_in_court = customer"
        in block
    )
    region = block.split("\n  chat_awaiting_customer:\n", 1)[1].split(
        "\n  # Peer-pack conversation upgrade (cycle 2023)", 1
    )[0]
    assert "source: Comment" in region
    assert "filter: channel = chat and ball_in_court = customer and is_internal = false" in region
    assert "display: conversation" in region
    assert "ball_in_court = agent" not in region
    assert "customer_tone" not in region
    assert "escalation" not in region
    ...
    manager = text.split("workspace manager_ops", 1)[1].split("workspace agent_dashboard", 1)[0]
    assert "\n  chat_awaiting_customer:\n" in manager
    assert "chat_awaiting_customer" in manager.split("focus:", 1)[1].split("\n", 1)[0]
    assert (
        "chat_awaiting_customer: count(Comment where channel = chat and ball_in_court = customer"
        in manager
    )
...
def test_ticket_queue_phone_awaiting_customer_trail() -> None:
    """Cycle 2023: Zendesk/Front phone still waiting on customer (channel×customer-ball).

    channel=phone AND ball_in_court=customer — not full phone_live, not agent
    phone_needs_reply, not chat_awaiting_customer re-stack.
    """
    text = APP.read_text()
    block = text.split("workspace ticket_queue", 1)[1].split("workspace manager_ops", 1)[0]
    assert (
        "phone_awaiting_customer: count(Comment where channel = phone and ball_in_court = customer"
        in block
    )
    region = block.split("\n  phone_awaiting_customer:\n", 1)[1].split(
        "\n  # Peer-pack conversation upgrade (cycle 1984)", 1
    )[0]
    assert "source: Comment" in region
    assert "filter: channel = phone and ball_in_court = customer and is_internal = false" in region
    assert "display: conversation" in region
    assert "ball_in_court = agent" not in region
    assert "customer_tone" not in region
    assert "escalation" not in region
    ...
    manager = text.split("workspace manager_ops", 1)[1].split("workspace agent_dashboard", 1)[0]
    assert "\n  phone_awaiting_customer:\n" in manager
    assert "phone_awaiting_customer" in manager.split("focus:", 1)[1].split("\n", 1)[0]
    assert (
        "phone_awaiting_customer: count(Comment where channel = phone and ball_in_court = customer"
        in manager
    )
...
def test_ticket_queue_portal_awaiting_customer_trail() -> None:
    """Cycle 2029: Intercom/Zendesk portal still waiting on customer (channel×customer-ball).

    channel=portal AND ball_in_court=customer — not full portal_live, not agent
    portal_needs_reply, not phone/chat/email_awaiting_customer re-stack.
    """
    text = APP.read_text()
    block = text.split("workspace ticket_queue", 1)[1].split("workspace manager_ops", 1)[0]
    assert (
        "portal_awaiting_customer: count(Comment where channel = portal and ball_in_court = customer"
        in block
    )
    region = block.split("\n  portal_awaiting_customer:\n", 1)[1].split(
        "\n  # Peer-pack conversation upgrade (cycle 1955)", 1
    )[0]
    assert "source: Comment" in region
    assert "filter: channel = portal and ball_in_court = customer and is_internal = false" in region
    assert "display: conversation" in region
    assert "ball_in_court = agent" not in region
    assert "customer_tone" not in region
    assert "escalation" not in region
    assert "channel = phone" not in region
    assert "channel = chat" not in region
    assert "channel = email" not in region
    manager = text.split("workspace manager_ops", 1)[1].split("workspace agent_dashboard", 1)[0]
    assert "\n  portal_awaiting_customer:\n" in manager
    assert "portal_awaiting_customer" in manager.split("focus:", 1)[1].split("\n", 1)[0]
    assert (
        "portal_awaiting_customer: count(Comment where channel = portal and ball_in_court = customer"
        in manager
    )
...
def test_ticket_queue_thankful_needs_reply_trail() -> None:
    """Cycle 2032: Intercom/Zendesk warm closeout still waiting on agent (tone×agent-ball).

    customer_tone=thankful AND ball_in_court=agent — not full thankful_recovery,
    not frustrated/urgent needs_reply heat re-stack, not channel×ball coat.
    """
    text = APP.read_text()
    block = text.split("workspace ticket_queue", 1)[1].split("workspace manager_ops", 1)[0]
    assert (
        "thankful_needs_reply: count(Comment where customer_tone = thankful and ball_in_court = agent"
        in block
    )
    ...
    assert "display: conversation" in region
    assert "ball_in_court = customer" not in region
    assert "customer_tone = frustrated" not in region
    assert "customer_tone = urgent" not in region
    assert "channel =" not in region
    assert "escalation" not in region
    manager = text.split("workspace manager_ops", 1)[1].split("workspace agent_dashboard", 1)[0]
    assert "\n  thankful_needs_reply:\n" in manager
    assert "thankful_needs_reply" in manager.split("focus:", 1)[1].split("\n", 1)[0]
    assert (
        "thankful_needs_reply: count(Comment where customer_tone = thankful and ball_in_court = agent"
        in manager
    )
    ...
    tnr = [
        r
        for r in rows
        if r.get("customer_tone") == "thankful"
        and r.get("ball_in_court") == "agent"
        and r.get("is_internal") is not True
    ]
    assert len(tnr) >= 3
...
def test_ticket_queue_thankful_awaiting_customer_trail() -> None:
    """Cycle 2035: Intercom/Zendesk warm closeout still waiting on customer (tone×customer-ball).

    customer_tone=thankful AND ball_in_court=customer — not full thankful_recovery,
    not agent thankful_needs_reply, not frustrated/urgent awaiting re-stack.
    """
    text = APP.read_text()
    block = text.split("workspace ticket_queue", 1)[1].split("workspace manager_ops", 1)[0]
    assert (
        "thankful_awaiting_customer: count(Comment where customer_tone = thankful and ball_in_court = customer"
        in block
    )
    region = block.split("\n  thankful_awaiting_customer:\n", 1)[1].split(
        "\n  # Peer-pack conversation upgrade (cycle 2029)", 1
    )[0]
    assert "source: Comment" in region
    assert (
    ...
    manager = text.split("workspace manager_ops", 1)[1].split("workspace agent_dashboard", 1)[0]
    assert "\n  thankful_awaiting_customer:\n" in manager
    assert "thankful_awaiting_customer" in manager.split("focus:", 1)[1].split("\n", 1)[0]
    assert (
        "thankful_awaiting_customer: count(Comment where customer_tone = thankful and ball_in_court = customer"
        in manager
    )
...
def test_ticket_queue_breach_awaiting_customer_trail() -> None:
    """Cycle 2038: Zendesk/Front SLA-pressure speech parked on customers (clock still burns).

    sla_pressure at_risk|breached AND ball_in_court=customer — not agent breach_needs_reply,
    not Ticket breach_risk rows, not ball-only awaiting_customer, not tone/channel×customer.
    """
    text = APP.read_text()
    block = text.split("workspace ticket_queue", 1)[1].split("workspace manager_ops", 1)[0]
    assert (
        "breach_awaiting_customer: count(Comment where (sla_pressure = at_risk or sla_pressure = breached)"
        in block
    )
    ...
    manager = text.split("workspace manager_ops", 1)[1].split("workspace agent_dashboard", 1)[0]
    assert "\n  breach_awaiting_customer:\n" in manager
    assert "breach_awaiting_customer" in manager.split("focus:", 1)[1].split("\n", 1)[0]
    assert (
        "breach_awaiting_customer: count(Comment where (sla_pressure = at_risk or sla_pressure = breached)"
        in manager
    )
...
def test_comment_declares_case_priority() -> None:
    """Cycle 2040: denormalized ticket priority on speech for priority×ball grain."""
    text = APP.read_text()
    block = text.split('entity Comment "Comment":', 1)[1].split("entity ", 1)[0]
...
def test_ticket_queue_priority_needs_reply_trail() -> None:
    """Cycle 2040: Zendesk/Front high-priority speech waiting-on-you (priority×ball).

    case_priority high|critical AND ball_in_court=agent — not Ticket open_queue
    priority rows, not ball-only needs_reply, not escalation×ball critical_needs_reply,
    not sla_pressure breach_needs_reply, not tone×ball re-stack.
    """
    text = APP.read_text()
    block = text.split("workspace ticket_queue", 1)[1].split("workspace manager_ops", 1)[0]
    assert (
        "priority_needs_reply: count(Comment where (case_priority = high or case_priority = critical) and ball_in_court = agent"
        in block
    )
    region = block.split("\n  priority_needs_reply:\n", 1)[1].split(
        "\n  # Peer-pack conversation upgrade (cycle 2042)", 1
    )[0]
    assert "source: Comment" in region
    assert (
    ...
    assert "display: conversation" in region
    # Priority×ball — not SLA-pressure, not escalation, not tone, not channel.
    assert "sla_pressure" not in region
    assert "escalation" not in region
    assert "customer_tone" not in region
    assert "channel =" not in region
    manager = text.split("workspace manager_ops", 1)[1].split("workspace agent_dashboard", 1)[0]
    assert "\n  priority_needs_reply:\n" in manager
    assert (
        "priority_needs_reply: count(Comment where (case_priority = high or case_priority = critical)"
        in manager
    )
    assert (
        "focus: at_risk_needs_reply, breached_needs_reply, media_shelf, queue_metrics, needs_reply, medium_needs_reply, priority_needs_reply, priority_awaiting_customer, breach_awaiting_customer, breach_needs_reply, thankful_needs_reply, thankful_awaiting_customer, portal_awaiting_customer, phone_awaiting_customer, chat_awaiting_customer, email_awaiting_customer, critical_awaiting_customer, raised_awaiting_customer, live_conversation"
    ...
    pnr = [
        r
        for r in rows
        if r.get("case_priority") in {"high", "critical"}
        and r.get("ball_in_court") == "agent"
        and r.get("is_internal") is not True
    ]
    assert len(pnr) >= 3
    # All seeds carry case_priority
    assert all("case_priority" in r for r in rows)


def test_ticket_queue_priority_awaiting_customer_trail() -> None:
    """Cycle 2042: Zendesk/Front high-priority speech parked on customer (priority×ball).

    case_priority high|critical AND ball_in_court=customer — not agent priority_needs_reply,
    not ball-only awaiting_customer, not Ticket open_queue priority rows, not escalation/SLA×customer.
    """
    text = APP.read_text()
    block = text.split("workspace ticket_queue", 1)[1].split("workspace manager_ops", 1)[0]
    assert (
        "priority_awaiting_customer: count(Comment where (case_priority = high or case_priority = critical) and ball_in_court = customer"
        in block
    )
    region = block.split("\n  priority_awaiting_customer:\n", 1)[1].split(
        "\n  # Peer-pack conversation upgrade (cycle 2032)", 1
    )[0]
    assert "source: Comment" in region
    assert (
    ...
    assert "\n  priority_awaiting_customer:\n" in manager
    assert (
        "priority_awaiting_customer: count(Comment where (case_priority = high or case_priority = critical)"
        in manager
    )
    assert (
        "focus: at_risk_needs_reply, breached_needs_reply, media_shelf, queue_metrics, needs_reply, medium_needs_reply, priority_needs_reply, priority_awaiting_customer, breach_awaiting_customer, breach_needs_reply, thankful_needs_reply, thankful_awaiting_customer, portal_awaiting_customer, phone_awaiting_customer, chat_awaiting_customer, email_awaiting_customer, critical_awaiting_customer, raised_awaiting_customer, live_conversation"
    ...
    # Agent grain precedes customer-parked priority grain.
    assert block.index("\n  priority_needs_reply:\n") < block.index(
        "\n  priority_awaiting_customer:\n"
    )
    rows = [json.loads(line) for line in NOTE_SEEDS.read_text().splitlines() if line.strip()]
...
def test_ticket_queue_medium_needs_reply_trail() -> None:
    """Cycle 2060: case_priority=medium AND ball_in_court=agent — not high|critical priority_needs_reply."""
    text = APP.read_text()
    block = text.split("workspace ticket_queue", 1)[1].split("workspace manager_ops", 1)[0]
    assert (
        "medium_needs_reply: count(Comment where case_priority = medium and ball_in_court = agent"
        in block
    )
    region = block.split("\n  medium_needs_reply:\n", 1)[1].split(
        "\n  priority_awaiting_customer:", 1
    )[0]
    assert "case_priority = medium" in region
    assert "ball_in_court = agent" in region
    assert "display: conversation" in region
    manager = text.split("workspace manager_ops", 1)[1].split("workspace agent_dashboard", 1)[0]
    assert "\n  medium_needs_reply:\n" in manager
    assert "medium_needs_reply" in manager.split("focus:", 1)[1].split("\n", 1)[0]
    assert "medium_needs_reply" in block.split("focus:", 1)[1].split("\n", 1)[0]
    assert block.index("\n  needs_reply:\n") < block.index("\n  medium_needs_reply:\n")
    assert block.index("\n  medium_needs_reply:\n") < block.index("\n  priority_needs_reply:\n")
    rows = [json.loads(line) for line in NOTE_SEEDS.read_text().splitlines() if line.strip()]
    mid = [
...
def test_ticket_queue_sla_pressure_stage_density() -> None:
    """Cycle 2075 recipe sla_pressure_stage_density — exclusive at-risk vs breached needs-reply."""
    text = APP.read_text()
    block = text.split("workspace ticket_queue", 1)[1].split("workspace manager_ops", 1)[0]
    soft = block.split("\n  at_risk_needs_reply:\n", 1)[1].split("\n  breached_needs_reply:", 1)[0]
    hard = block.split("\n  breached_needs_reply:\n", 1)[1].split("\n  breach_needs_reply:", 1)[0]
    assert "source: Comment" in soft
    assert "sla_pressure = at_risk" in soft
    assert "ball_in_court = agent" in soft
    assert "display: conversation" in soft
    assert "sla_pressure = breached" not in soft
    assert "source: Comment" in hard
    assert "sla_pressure = breached" in hard
    assert "ball_in_court = agent" in hard
    assert "display: conversation" in hard
    assert "sla_pressure = at_risk" not in hard
    assert "at_risk_needs_reply: count(Comment where sla_pressure = at_risk" in block
    assert "breached_needs_reply: count(Comment where sla_pressure = breached" in block
    assert "at_risk_needs_reply" in block.split("focus:", 1)[1].split("\n", 1)[0]
    assert "breached_needs_reply" in block.split("focus:", 1)[1].split("\n", 1)[0]
    assert "sla_pressure_stage_density" in block or "at-risk vs breached" in block.lower()
