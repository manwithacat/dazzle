...
def test_pdf_controller_leftover_page_does_not_invent() -> None:
    """dz-pdf.js must refuse leftover page junk (2151)."""
    from pathlib import Path
    ...
    assert "function parsePage" in src
    assert "must not invent" in src
    assert "leftover junk stays visible" in src


def test_pdf_controller_leftover_zoom_does_not_invent() -> None:
    """dz-pdf.js must refuse leftover zoom junk (2152)."""
    from pathlib import Path
    ...
    assert "function parseZoom" in src
    assert "leftoverZoomMessage" in src
    assert "?dzpdf-zoom leftover" in src
    assert "must not invent" in src
    assert "leftover junk stays visible" in src
...
def test_grid_edit_controller_leftover_number_does_not_invent() -> None:
    """dz-grid-edit.js must refuse leftover number junk (2155)."""
    from pathlib import Path
    ...
    assert "function numberLeftoverBlocksCommit" in src
    assert "function leftoverBlocksCommit" in src
    assert "data-dz-number-value" in src
    assert "must not invent a number commit" in src
