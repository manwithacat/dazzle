...
- _connect method returns appropriate connection type (mock asyncpg)
...
class TestVersionManagerPgConnect:
    ...
    @pytest.mark.asyncio
    async def test_connect_returns_asyncpg_connection(self) -> None:
        """Test that _connect returns asyncpg connection in pg mode."""
        vm = VersionManager(db_path="test.db", database_url="postgresql://localhost/test")
...
class TestVersionManagerPgOperations:
    ...
    async def test_deploy_version_postgres(self) -> None:
        """Test deploy_version in Postgres mode uses $1 placeholders."""
        vm = VersionManager(
            db_path="test.db",
            database_url="postgresql://localhost/test",
        )
        ...
        # Should have checked existence and inserted
    ...
    async def test_get_current_version_postgres(self) -> None:
        ...
        mock_conn = AsyncMock()
        mock_conn.fetchrow = AsyncMock(return_value={"version_id": "v1"})
        mock_conn.close = AsyncMock()
