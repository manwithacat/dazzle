...
| **`HM hyperpart coherence`** | Full gallery investigate **or** drain per `improve/strategies/hyperpart_coherence.md`. Prefer **drain** when `python scripts/hm_coherence_queue.py --status` shows `queue>0`; else investigate (`hm_pages_vision.py --all-hyperparts` + subagent batches + ingest + `--seed-backlog`). | 1 | USED@N on investigate or drain completion |
