...
- Batch MCP operations for autonomous agent loops ([#165](https://github.com/manwithacat/dazzle/issues/165))
...
- HX-Trigger response headers for server-to-client event coordination ([#142](https://github.com/manwithacat/dazzle/issues/142))
...
- CLI command activity now written to shared activity store for Workshop visibility ([#225](https://github.com/manwithacat/dazzle/issues/225))
- UK government identifier patterns (NINO, UTR) added to compliance scanner ([#221](https://github.com/manwithacat/dazzle/issues/221))
- Discovery agent switched to MCP sampling instead of direct Anthropic API calls ([#220](https://github.com/manwithacat/dazzle/issues/220))
...
- Default Docker setup upgraded to production-grade local infrastructure ([#198](https://github.com/manwithacat/dazzle/issues/198))
- Orphan experiences with no workspace entry point now detected in discovery ([#194](https://github.com/manwithacat/dazzle/issues/194))
- `composition(audit)` step added to the pipeline ([#192](https://github.com/manwithacat/dazzle/issues/192))
...
- Perfect-score surfaces omitted from pipeline fidelity output to reduce size ([#189](https://github.com/manwithacat/dazzle/issues/189))
- Hero-balance severity escalated when media is declared but not rendered side-by-side ([#188](https://github.com/manwithacat/dazzle/issues/188))
...
- Summary mode for pipeline and fidelity producing LLM-friendly compact output ([#173](https://github.com/manwithacat/dazzle/issues/173))
- Money field widget with major/minor unit conversion in forms ([#172](https://github.com/manwithacat/dazzle/issues/172))
- SQLite removed from runtime; PostgreSQL is now the only backend ([#158](https://github.com/manwithacat/dazzle/issues/158))
- Migrated from psycopg2 + asyncpg to unified psycopg v3 ([#155](https://github.com/manwithacat/dazzle/issues/155))
- PostgreSQL-first for all runtime and testing paths ([#154](https://github.com/manwithacat/dazzle/issues/154))
- Postgres constraint violations now return 422 with helpful messages ([#146](https://github.com/manwithacat/dazzle/issues/146))
- htmx upgraded from 2.0.3 to 2.0.8 ([#144](https://github.com/manwithacat/dazzle/issues/144))
- Idiomorph swap strategy for improved table update performance ([#143](https://github.com/manwithacat/dazzle/issues/143))
- Alpine.js replaced with purpose-built `dz.js` micro-runtime ([#141](https://github.com/manwithacat/dazzle/issues/141))

### Deprecated
- Dazzle Bar dev toolbar removed ([#164](https://github.com/manwithacat/dazzle/issues/164))

### Fixed
- `dsl-run --base-url` now authenticates before running CRUD tests ([#228](https://github.com/manwithacat/dazzle/issues/228))
- Workshop showing no activity — JSONL writer connected to SQLite store ([#223](https://github.com/manwithacat/dazzle/issues/223))
- v0.25.0 constructs silently dropped by pre-v0.25.0 dispatchers ([#222](https://github.com/manwithacat/dazzle/issues/222))
- MCP validate accepting DSL with reserved keyword conflicts ([#219](https://github.com/manwithacat/dazzle/issues/219))
- Remaining ForwardRef modules causing `/openapi.json` 500 errors ([#218](https://github.com/manwithacat/dazzle/issues/218))
- Pydantic ForwardRef resolution failure breaking `/openapi.json` ([#217](https://github.com/manwithacat/dazzle/issues/217))
- `create_sessions` `asyncio.run()` failure inside MCP event loop ([#216](https://github.com/manwithacat/dazzle/issues/216))
- Event publisher `InFailedSqlTransaction` cascade that floods logs ([#215](https://github.com/manwithacat/dazzle/issues/215))
- Surface converter now uses proper English pluralization via `to_api_plural()` ([#214](https://github.com/manwithacat/dazzle/issues/214))
- `from __future__ import annotations` removed from 9 route modules fixing `/openapi.json` ([#213](https://github.com/manwithacat/dazzle/issues/213))
- `/openapi.json` returning 500 Internal Server Error ([#211](https://github.com/manwithacat/dazzle/issues/211))
- `dsl_test` `create_sessions` `asyncio.run()` from running event loop ([#210](https://github.com/manwithacat/dazzle/issues/210))
- Workspace routes returning 404 despite nav links pointing to them ([#209](https://github.com/manwithacat/dazzle/issues/209))
- Naive entity pluralization producing incorrect API route paths ([#208](https://github.com/manwithacat/dazzle/issues/208))
- `dsl_test` using wrong API route pattern causing all CRUD tests to fail ([#207](https://github.com/manwithacat/dazzle/issues/207))
- Pipeline story/process coverage pagination hiding partial stories ([#193](https://github.com/manwithacat/dazzle/issues/193))
- Compliance signal false positives on non-PII fields like `ip_address` and rule name ([#191](https://github.com/manwithacat/dazzle/issues/191))
- `composition(analyze)` LLM vision path not executing (0 tokens used) ([#184](https://github.com/manwithacat/dazzle/issues/184))
- Sitespec rendering for `split_content` source.path, pricing layout, and tiers ([#182](https://github.com/manwithacat/dazzle/issues/182))
- Composition capture using Playwright sync API inside asyncio loop ([#181](https://github.com/manwithacat/dazzle/issues/181))
- Hero section media image not rendered despite correct sitespec configuration ([#179](https://github.com/manwithacat/dazzle/issues/179))
- Partial rendering for `card_grid`, `split_content`, `trust_bar`, and other section types ([#177](https://github.com/manwithacat/dazzle/issues/177))
- `policy simulate` evaluating `role()` conditions instead of always showing allow ([#176](https://github.com/manwithacat/dazzle/issues/176))
- Headless discovery flagging system entities as missing create/edit surfaces ([#175](https://github.com/manwithacat/dazzle/issues/175))
- Sitespec validator now recognises standalone entity routes ([#174](https://github.com/manwithacat/dazzle/issues/174))
- Fidelity checker false positives on money field expanded column names ([#171](https://github.com/manwithacat/dazzle/issues/171))
- CSS/HTML class name mismatches breaking site styling ([#166](https://github.com/manwithacat/dazzle/issues/166))
- Fresh deploy crash from missing SQLAlchemy base dependency ([#163](https://github.com/manwithacat/dazzle/issues/163))
- Migration crash on fresh PostgreSQL — FK topological sort added ([#162](https://github.com/manwithacat/dazzle/issues/162))
- `dazzle migrate` CLI now respects `DATABASE_URL` environment variable ([#161](https://github.com/manwithacat/dazzle/issues/161))
- `dazzle check` subprocess command including extra `dazzle` argument ([#160](https://github.com/manwithacat/dazzle/issues/160))
- Entity list routes 500 from `dict_row` incompatibility with psycopg v3 ([#157](https://github.com/manwithacat/dazzle/issues/157))
- PostgresBus startup crash from `.format()` conflicting with JSONB DEFAULT ([#156](https://github.com/manwithacat/dazzle/issues/156))
- FK pre-validation using SQLite placeholder on PostgreSQL ([#152](https://github.com/manwithacat/dazzle/issues/152))
- psycopg2 subclass exceptions bypassing `IntegrityError` check ([#151](https://github.com/manwithacat/dazzle/issues/151))
- DaisyUI drawer not accessible on iPad/tablet viewport ([#150](https://github.com/manwithacat/dazzle/issues/150))
- Static images returning 404 after unified server change ([#149](https://github.com/manwithacat/dazzle/issues/149))
- `dz.js` and `dz.css` returning 404 breaking sidebar drawer and dark mode ([#147](https://github.com/manwithacat/dazzle/issues/147))
- Workspace dashboards accessible without authentication ([#145](https://github.com/manwithacat/dazzle/issues/145))
- Enum validation error handler crash from non-serializable `ValueError` ([#140](https://github.com/manwithacat/dazzle/issues/140))
- `money(GBP)` expansion leaving orphaned column causing NOT NULL violation ([#139](https://github.com/manwithacat/dazzle/issues/139))
- Naive pluralization producing wrong route names ([#136](https://github.com/manwithacat/dazzle/issues/136))
- Stored XSS via unsanitized string/text fields ([#135](https://github.com/manwithacat/dazzle/issues/135))
- Unique constraint violations returning 500 instead of 422 ([#134](https://github.com/manwithacat/dazzle/issues/134))
- Foreign key constraint violations returning 500 instead of 422 ([#133](https://github.com/manwithacat/dazzle/issues/133))
- `auto_add`/`auto_update` modifiers not populating timestamps ([#132](https://github.com/manwithacat/dazzle/issues/132))
- `money(GBP)` type not expanding into `_minor`/`_currency` column pair ([#131](https://github.com/manwithacat/dazzle/issues/131))
- Enum field values not validated against DSL-defined options ([#130](https://github.com/manwithacat/dazzle/issues/130))
