...
- [x] **Step 2 (revised) — DONE (James: "Do Task 6 now").** (a) Threaded `services` + the resolved `surface_name` through `compile_experience_context` → `ExperienceContext` → the http experience route, which pre-renders the table-step list via `dispatch_render` (in **http**, respecting `page ↛ http`) and passes the HTML to `render_experience_inner_html(..., table_step_html=)`; the page renderer shows a loud placeholder when the route can't render (no services/surface). Added `tests/integration/test_experience_table_step_substrate.py` (the previously-untested ops_dashboard `incident_response` triage step → substrate list + the loud-placeholder path). (b) `_render_body_inner`'s list branch now raises a loud `RuntimeError` (D4) instead of an empty 200. (c) Deleted `src/dazzle/page/runtime/table_renderer.py` (whole module, all 4 fns); repointed template_renderer; updated `fragment_registry` (stale strings → substrate modules); migrated `test_create_cta_entity_title_1487` / `test_surface_refresh_1399` / `test_typed_runtime_no_jinja` / `test_runtime_pipeline` / `test_page_purpose_wiring`; deleted `test_legacy_list_chrome_char_phase1` + snapshots; regen complexity + deferred-imports baselines.
- [x] **Step 3: FULL suite + gates** — 19257 green; mypy clean; import-linter 6 kept (`render is pure` + `page ↛ http` HELD).
- [x] **Step 4: Commit + ship.**

## Task 7: ADR status + CHANGELOG + ship

- [x] **Step 1:** ADR-0049 status → Phase 1 (list) SHIPPED. CHANGELOG + Agent Guidance.
- [x] **Step 2:** `/bump patch`; full suite green; push; monitor CI.

## Phase 1 gate
- [x] All list surfaces first-paint via the substrate (skeleton+hydrate); `render_data_row` the sole row source; `render_filterable_table` DELETED; full unit suite (19257) + card-safety composite + structural a11y scanners green (browser `ux verify` deferred — needs live serve+DB, flagged for CI/e2e); `render is pure` + `page ↛ http` KEPT; no page-level byte-goldens needed re-baselining (substring-asserted). **ADR-0049 Phase 1 DONE.**
