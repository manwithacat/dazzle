...
New `via` keyword in scope conditions:
...
`ViaCondition` sits alongside `Comparison` and `LogicalExpr` in the condition expression union. A `ScopeRule` with a `ViaCondition` means the rule's condition is a subquery.
...
`_convert_access_condition()` in `entity_converter.py` gets a new branch: when the condition is a `ViaCondition`, produce a `ViaConditionSpec`. Bindings map 1:1.
...
2. Builds WHERE clauses from user bindings and literal filters
3. Returns parameterized SQL — user values go through `$N` placeholders, identifiers through `quote_id()`

### Repository Layer

The filter dict uses a new key suffix `__in_subquery` which the repository interprets as `WHERE "field" IN (subquery)`. Example:
