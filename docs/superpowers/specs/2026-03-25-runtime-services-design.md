...
- `dazzle` package singletons: `rbac/audit.py`, `mcp/server/state.py`, `runtime_tools/state.py` (no FastAPI access — need different pattern)
...
**`EventBusAwareRepository`** — takes `event_bus` as constructor arg instead of calling `get_event_bus()`:
```python
class EventBusAwareRepository:
...
| `events/framework.py` | `_framework` global, `get_event_framework()` |
...
| `runtime/metrics/emitter.py` | `_emitter` global, `get_emitter()` |
...
| `runtime/subsystems/events.py:61` | implicit framework global | `services.event_framework` |
...
| `runtime/metrics/emitter.py:235` | `get_emitter()` | `services.metrics_emitter` |
...
| `runtime/event_bus.py:418` | `get_event_bus()` fallback | `self._event_bus` (required param) |
...
| `src/dazzle_back/runtime/event_bus.py` | Delete global singleton functions; keep `EntityEventBus` class |
| `src/dazzle_back/runtime/presence_tracker.py` | Delete global singleton functions; keep `PresenceTracker` class |
| `src/dazzle_back/events/framework.py` | Delete `_framework` global; `init_framework()` returns framework |
...
| `src/dazzle_back/runtime/metrics/emitter.py` | Delete `_emitter` global and getter |
...
| `src/dazzle_back/runtime/subsystems/events.py` | Pass `services.event_framework` to service mixin |
| `src/dazzle_back/runtime/event_bus.py` (`EventBusAwareRepository`) | Take `event_bus` as constructor arg |
