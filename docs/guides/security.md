...
- **Layer 3 — Audit trail:** Every access decision for `audit:`-declared
  ...
  (`src/dazzle/back/runtime/audit_log.py`).
...
| **Input validation & XSS** | Pydantic validates every API request body against generated schemas. Jinja2 template auto-escaping prevents XSS in server-rendered HTML. Query parameters used in HTML attributes are escaped via `html.escape` in the route generator. | Do not render user-controlled data via `| safe` or `Markup()` in custom templates without sanitisation. Validate business constraints beyond type-checking (e.g. enum ranges, numeric bounds) in service blocks. | No server-side input schema validation beyond what Pydantic derives from the DSL IR types. Custom service blocks that accept free-form input must validate it themselves. |
...
**Dazzle's mitigation:** `CSRFMiddleware` in `src/dazzle/back/runtime/csrf.py`
...
`SECURITY BARRIER` view, no append-only trigger, no hash-chain, and no external
log sink. An attacker with `DELETE` privilege on the table can erase records
without leaving a trace in the audit log itself. If you require tamper-resistant
audit evidence (e.g. for SOC 2 or ISO 27001 A.8.15), stream the audit log to an
immutable external sink (e.g. CloudWatch Logs, Splunk, BigQuery append-only
table). This is documented as a gap in section 4.
...
`src/dazzle/back/runtime/workspace_region_handler.py` calls
