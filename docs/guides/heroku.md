...
## Deploy Files (uv buildpack)

Heroku's Python buildpack [natively supports uv](https://devcenter.heroku.com/changelog-items/3238) (since May 2025), which gives reproducible, hash-pinned builds instead of re-resolving dependencies on every deploy. Dazzle scaffolds the uv layout for you:

```bash
dazzle deploy heroku        # writes Procfile, pyproject.toml, uv.lock, .python-version
```

| File | Purpose |
|------|---------|
| `pyproject.toml` | declares `dazzle-dsl[serve]` (pinned to your installed version) + Postgres/Redis deps; `requires-python = ">=3.12"`; `[tool.uv] package = false` (a DSL app, not a wheel) |
| `uv.lock` | hash-pinned resolution of the full tree (generated by `uv lock`) |
| `.python-version` | `3.14` — primary interpreter Heroku provisions (floor remains `requires-python >=3.12`) |
| `Procfile` | `web: dazzle serve --production` |

Commit all four, then deploy:
...
> **uv vs pip.** Heroku activates the uv path only when `pyproject.toml` + `uv.lock` + `.python-version` are present **and** `requirements.txt`/`Pipfile`/`poetry.lock` are absent. Migrating from the old pip layout? Delete `requirements.txt` and `runtime.txt` first (`dazzle deploy heroku` warns you if they're still present). To stay on pip instead, run `dazzle deploy heroku --pip` (emits `requirements.txt` + `runtime.txt`).

After changing dependencies, re-run `uv lock` and commit the updated `uv.lock` — Heroku installs from the lock, not a live resolve.

## Procfile

`dazzle deploy heroku` writes a `web: dazzle serve --production` Procfile, which is enough for most apps. For explicit multi-worker control (and the `WEB_CONCURRENCY` pool math below), use the uvicorn form instead:
