...
|---|---|---|---|---|---|
| **1a** | region form inference | 2→4 | **`display: auto`** — infer the region form from the source's data shape. **Opt-in resolver SHIPPED** (#1492, v0.88.3): `resolve_auto_display(region, entities)` unifies the EX-047/#1082 aggregate→SUMMARY + kanban promotions and adds the temporal→timeline rule; reachable via `display: auto`. **Remaining (2→3→4):** make `auto` the *default* for unset `display:` — validated against the example fleet + visual baselines (the level-moving, higher-risk step). | S | — |
