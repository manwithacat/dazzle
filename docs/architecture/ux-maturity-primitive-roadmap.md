...
Read against the htmx-4 (`four-dev`, pin `HTMX_PINNED_VERSION` / currently 4.0.0) source + extension set. The
...
| **1c** | comparison context | ~~4~~ **2** | **#1678 reversed the #1491 default.** Unset `metrics`/`summary` tiles are value+tone only; authors opt in with `delta:` (#884). Inferred 30-day sparks lied on ops boards whose book is shorter than the window (BioChart `week_energy`). Empty prior omits the arrow. Capability exists (L2); it is not the default. | S | — |
