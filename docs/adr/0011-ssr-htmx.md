...
>
> **htmx version note (2026-06-17; pin updated 2026-09-07):** Migrated htmx 2.0.9 → **htmx 4** (first ship **4.0.0-beta4**, GA **4.0.0** in #1409; evaluation in `docs/evaluation/htmx4-evaluation.md` + `htmx4-browser-baseline.md`). Pin is `HTMX_PINNED_VERSION` in `scripts/update_vendors.py` — not npm `latest` (still 2.x) and not GitHub `/releases/latest` (4.x betas were mislabelled). SSR+HTMX decision unchanged; this is a runtime-library upgrade. Key shifts absorbed: colon-namespaced events (`htmx:before:request` etc.); fetch replaces XHR (`detail.ctx.response.status` / `.headers.get()`, CSRF via `detail.ctx.request.headers`); native morph (`hx-swap="innerMorph"`, replaces the idiomorph extension); explicit attribute inheritance (bridged with `htmx.config.implicitInheritance=true` pending a markup audit); all htmx-2 extension *files* dropped (native morph/`hx-status`/`hx-sse` + server-side `HX-Retarget` cover their roles; forms send url-encoded which the server parser already accepts, so json-enc is unnecessary). **Two htmx-4 behavioural traps, both the same root cause** — implicit default triggers now re-fire on swap/reprocess (filter-select self-swap loop; body-drill `<tr>` auto-navigation). Fix pattern: make triggers explicit (`hx-trigger="change changed"` / `hx-trigger="click"`). Verified regression-free via `dazzle ux verify --interactions` on the 3 workspace apps.
