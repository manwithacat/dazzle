...
**Status:** Accepted 2026-06-01 — implementation **staged** (#1319). Slice A (surface: `invoke_flow` IR + parser + validator + `execute_atomic_flow_on_conn` seam + a state-machine fixture; binding declared + analyzable, not yet wired into the live update path) lands first; **Slice B** (the shared-transaction hot-path integration — `repository.update` conn-injection, `CRUDService.update` interception, `AuthContext`/`access_specs` threading + real-PG atomicity tests) is reviewed separately. *Code finding (exploration):* the status `UPDATE` currently commits inside `repository.update`'s own connection **before** transition effects fire (today's effects are post-commit), so the shared-tx atomicity below requires bringing the status write into the executor's transaction — a core-CRUD-update-path change, done in Slice B. ADR-0020 remains Proposed; the transition/state-machine runtime exists regardless.
