...
See `src/dazzle/http/migrations/` for Alembic environment. `env.py` imports the DSL-derived SQLAlchemy `MetaData` to enable autogenerate. Safe type casts are registered in `alembic/cast_rules.py`.
