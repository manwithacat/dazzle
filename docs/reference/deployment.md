...
- **Applying the policies in production:** the runtime applies RLS automatically only on the dev `create_all` path. In production (Alembic owns the schema) you must apply the generated policy DDL as a deploy step — run the output of `build_rls_policy_ddl(...)` against the database after migrations — until the migration + drift-gate integration lands (Phase D). Until then, app-layer scope filters remain the active tenant-isolation enforcement; the RLS fence is additive hardening.
