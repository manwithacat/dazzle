...
| UX-036 | auth-page          | READY_FOR_QA | DONE     | PARTIAL  | PENDING | —                 | login,signup,forgot_password,reset_password,2fa_*                | 6        | 2026-04-13 | 2fa_setup.html adopted (Cycle 41 — 6/7). Most complex adopter: outline primary + outline neutral + ghost-link button variants, plain `<hr>` replaces DaisyUI `.divider`, neutral callout for recovery codes, JS badge class string replaced with `RECOVERY_CODE_CLASSES` constant. **impl: PARTIAL** — 1 auth file remains: 2fa_settings |
