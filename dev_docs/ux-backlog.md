...
| UX-036 | auth-page          | READY_FOR_QA | DONE     | PARTIAL  | PENDING | —                 | login,signup,forgot_password,reset_password,2fa_*                | 5        | 2026-04-13 | 2fa_challenge.html adopted (Cycle 40 — 5/7). Pure-Tailwind replacement for DaisyUI `.divider` (centred "OR" over HR). OTP input uses h-10 + tracking-[0.4em] + text-center. Ghost-button variant for "send code". Recovery-code JS now targets `#dz-code-label` instead of `.label-text`. **impl: PARTIAL** — 2 auth files remain: 2fa_setup, 2fa_settings |
