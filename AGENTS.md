...
**Dazzle structural authoring stays in the agent session.** The MCP, KG, parser, IR types, examples, and this file all assume an in-session agent does DSL synthesis with full Dazzle context; an out-of-context API call can't produce idiomatic DSL.
...
## Autonomous Loops (awareness)

Dazzle has a single agent-first entrypoint for autonomous investigation, improvement, refactoring, and remediation: the improve loop. The driver picks the highest-leverage **lane** each cycle based on actionable rows and signals, then hands off to that lane's playbook:
...
| `demo_data` | get |
| `discovery` | coherence |
| `domain` | extract, show, gaps, research, promote |
...
| `presentation` | cognition, opportunities, residual, shapes |
...
| `status` | mcp, logs, active_project, telemetry, activity, demo_world, runtime |
...
---
**Version**: 0.113.11 | **Python**: 3.12+ | **Status**: Production Ready
