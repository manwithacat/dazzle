...
story ST-018 "User creates a new Comment":
  ...
  then:
    ...
    - "User sees confirmation message"
    - "Comment activity list can open the note detail, parent Ticket hub, or author User (triple open)"
...
story ST-019 "Support Agent works the open ticket queue":
  ...
  then:
    ...
    - "Ticket rows triple-open Ticket via id | User via assigned_to | User via created_by (ticket hub first, assignee then creator)"
...
story ST-028 "Support Manager reassigns a ticket":
  ...
  then:
    ...
    - "From the ticket queue, triple-open can hop assignee or creator User hub before reassignment edit"
