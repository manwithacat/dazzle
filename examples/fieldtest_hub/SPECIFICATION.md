...
**Data & reliability.** All data is stored in PostgreSQL — a mature, widely-trusted relational database. There is no bespoke or experimental datastore to operate, secure, or reason about (verify: `dazzle db status`). In production, every change to the data model is applied through versioned, reversible migrations; the live schema is never edited by hand, so upgrades are repeatable and fully auditable (verify: `dazzle db status`). Money movement is recorded as balanced, double-entry transactions against declared ledger accounts: every movement has an equal and opposite entry, so the books cannot silently drift out of balance (verify: `dazzle validate`).
...
<!-- dazzle-spec-brief: sha256:4da97abbd2b7bec20fc7fbed0143d4132d2638e8705f0bab2f9625aa3cb2b0e4 -->
