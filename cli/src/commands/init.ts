...
import { runInitWizard, showProgress, showSuccess, showError, type ApiPack } from '../ui/prompts/init-wizard'
...
`,
  ...
  async run(args, ctx) {
    ...
    if (isInteractive) {
      // Fetch available API packs for integration selection
      let availablePacks: ApiPack[] = []
      try {
        const packsResult = await ctx.python<{ packs: ApiPack[] }>(
          'dazzle.core.cli_bridge',
          'list_api_packs_json',
          {}
        )
        if (packsResult.success && packsResult.data?.packs) {
          availablePacks = packsResult.data.packs
        }
      } catch {
        // Silently continue without packs
      }

      // Run interactive wizard with available packs
      const result = await runInitWizard(args.name, availablePacks)
          ...
          ? [
              {
                ...
                task: async () => {
                  const envResult = await ctx.python<{ content: string }>(
                    'dazzle.core.cli_bridge',
                    'generate_env_example_json',
                    { pack_names: result.integrations }
                  )
                  if (envResult.success && envResult.data?.content) {
                    const header = `# Environment Variables for ${result.name}\n# Generated by dazzle init\n\n`
                    await Bun.write(`${targetPath}/.env.example`, header + envResult.data.content)
                  }
                },
              },
            ]
