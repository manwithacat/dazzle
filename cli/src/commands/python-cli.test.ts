 ...
 * 2. Subcommand groups: vocab list, test run, etc.
 *
...
describe("Python CLI - Subcommand Groups", () => {
  ...
  const subcommandGroups = [
    // Runtime commands are now top-level (no longer under 'dnr' group)
...
describe("Python CLI - Invalid Command Patterns", () => {
  ...
  const invalidPatterns = [
    ...
    {
      args: ["layout", "plan"],
      description: "layout plan (should be layout-plan)",
    },
    {
      args: ["analyze", "spec"],
      description: "analyze spec (should be analyze-spec)",
    },
  ];
...
describe("Bun CLI Source - Command Argument Verification", () => {
  ...
  test("dev.ts uses correct serve subcommand", async () => {
    const source = await Bun.file(join(TEST_DIR, "dev.ts")).text();

    // Should use ['serve'] as the command
...
describe("Python CLI - Help Text Consistency", () => {
  ...
  test("Python CLI main help lists expected commands", async () => {
    ...
    // Check for runtime commands (now top-level)
